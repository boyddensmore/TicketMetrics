...
//Populate Siteid if null
var siteid = mbo.getMboValue("SITEID");

if (siteid.isNull() || siteid == undefined) {
	mbo.setValue("SITEID", "IT-IS");
}
...
{
	println (">>>>>  EX_SRSAVE | MAIN | No SLA on ticket.  Trying to apply SLA.");
	var slaRecordSet = mbo.getMboSet("REP_SLA");
	slaRecordSet.applySLA();
}
...
if (mbo.getMboValue("OWNERGROUP").isModified() && (!newOwnerGroup.equals(oldOwnerGroup))) {
	...
	if (EX_TICKETMETRICS.count() > 0) {
		...
		myLogger.debug(">>>>>  EX_SRSAVE | MAIN | EX_TICKETMETRICS record exists.  Updating it with response and resolution time.");
...
function calcTargResolutionDate(reportDate) {
	myLogger.debug(">>>>>  EX_SRSAVE | calcTargResolutionDate() | Begin");
...
function calcBusTime(startDate, endDate) {
	...
	myLogger.debug(">>>>>  EX_SRSAVE | calcBusTime() | Begin");
	...
	if (slaRecordSet.isEmpty() || slaRecordSet.count() < 1) {
		myLogger.warn(">>>>>  EX_SRSAVE | calcBusTime() | No SLA is applied, not calculating EX_TICKETMETRICS resolution details!");
	} else {
		myLogger.debug(">>>>>  EX_SRSAVE | calcBusTime() | SLA Set Count: " + slaRecordSet.count());

		var slaRecordMbo = slaRecordSet.getMbo(0);

		//myLogger.debug(">>>>>  EX_SRSAVE | calcBusTime() | slaRecordMbo.getMboValue(SLANUM): " + slaRecordMbo.getMboValue("SLANUM"));
		//myLogger.debug(">>>>>  EX_SRSAVE | calcBusTime() | slaRecordMbo.getMboValue(CALCORGID): " + slaRecordMbo.getMboValue("CALCORGID"));
		//myLogger.debug(">>>>>  EX_SRSAVE | calcBusTime() | slaRecordMbo.getMboValue(CALCCALENDAR): " + slaRecordMbo.getMboValue("CALCCALENDAR"));
		//myLogger.debug(">>>>>  EX_SRSAVE | calcBusTime() | slaRecordMbo.getMboValue(CALCSHIFT): " + slaRecordMbo.getMboValue("CALCSHIFT"));
		...
		myLogger.debug(">>>>>  EX_SRSAVE | calcBusTime() | finalvalue: " + finalvalue);
	...
	myLogger.debug(">>>>>  EX_SRSAVE | calcBusTime() | End");
	myLogger.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
}
...
function getHours(startDt, endDt, calName, shift, org) {
	//myLogger.debug(">>>>>  EX_SRSAVE | getHours() | Begin");

	//myLogger.debug(">>>>>  EX_SRSAVE | getHours() | startDt: " + startDt);
	//myLogger.debug(">>>>>  EX_SRSAVE | getHours() | endDt: " + endDt);
	//myLogger.debug(">>>>>  EX_SRSAVE | getHours() | calName: " + calName);
	//myLogger.debug(">>>>>  EX_SRSAVE | getHours() | shift: " + shift);
	//myLogger.debug(">>>>>  EX_SRSAVE | getHours() | org: " + org);
...
function getNonWrkMins(startDt, endDt) {
	//myLogger.debug(">>>>>  EX_SRSAVE | getNonWrkMins() | Begin");
...
function getStartHours(workDate, calName, shift, org) {
	...
	//myLogger.debug(">>>>>  EX_SRSAVE | getStartHours() | Begin");
...
function getEndHours(workDate, calName, shift, org) {
	...
	//myLogger.debug(">>>>>  EX_SRSAVE | getEndHours() | Begin");
...
function addTicketMetricRec() {
	myLogger.debug(">>>>>  EX_SRSAVE | addTicketMetricRec() | Begin");
...
function ISODateString(d) {
	//myLogger.debug(">>>>>  EX_SRSAVE | ISODateString() | Begin");
	function pad(n) {
		return n < 10 ? '0' + n : n
	}
