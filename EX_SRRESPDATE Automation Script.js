...
function calcBusTime(startDate, endDate) {
	...
	//myLogger.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
	myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | Begin");
	...
	if (slaRecordSet.isEmpty() || slaRecordSet.count() < 1) {
		myLogger.warn(">>>>>  EX_SRRESPDATE | calcBusTime() | No SLA is applied, not calculating TicketMetrics resolution details!");
	} else {
		myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | SLA Set Count: " + slaRecordSet.count());

		var slaRecordMbo = slaRecordSet.getMbo(0);

		// myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | slaRecordMbo.getMboValue(SLANUM): " + slaRecordMbo.getMboValue("SLANUM"));
		// myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | slaRecordMbo.getMboValue(CALCORGID): " + slaRecordMbo.getMboValue("CALCORGID"));
		// myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | slaRecordMbo.getMboValue(CALCCALENDAR): " + slaRecordMbo.getMboValue("CALCCALENDAR"));
		// myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | slaRecordMbo.getMboValue(CALCSHIFT): " + slaRecordMbo.getMboValue("CALCSHIFT"));
		...
		myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | totMins: " + totMins);

		/* Get the remaining hours on the start and end dates */
		var wrkHrsStart = getStartHours(startDate, varcal, varshift, varorg);
		// myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | wrkHrsStart: " + wrkHrsStart);
		if (wrkHrsStart > 0) {
		...
		// myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | wrkHrsEnd: " + wrkHrsEnd);
		...
		// myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | nonWrkMins: " + nonWrkMins);

		/* Subtract the nonwrk hrs from total */
		var finalvalue = (Math.abs(totMins - (startDateMins + endDateMins))) - (nonWrkMins);
		myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | finalvalue: " + finalvalue);
	...
	myLogger.debug(">>>>>  EX_SRRESPDATE | calcBusTime() | End");
	myLogger.debug(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
}
...
function getHours(startDt, endDt, calName, shift, org) {
	//myLogger.debug(">>>>>  EX_SRRESPDATE | getHours() | Begin");

	//myLogger.debug(">>>>>  EX_SRRESPDATE | getHours() | startDt: " + startDt);
	//myLogger.debug(">>>>>  EX_SRRESPDATE | getHours() | endDt: " + endDt);
	//myLogger.debug(">>>>>  EX_SRRESPDATE | getHours() | calName: " + calName);
	//myLogger.debug(">>>>>  EX_SRRESPDATE | getHours() | shift: " + shift);
	//myLogger.debug(">>>>>  EX_SRRESPDATE | getHours() | org: " + org);
...
function getNonWrkMins(startDt, endDt) {
	//myLogger.debug(">>>>>  EX_SRRESPDATE | getNonWrkMins() | Begin");
...
function getStartHours(workDate, calName, shift, org) {
	...
	//myLogger.debug(">>>>>  EX_SRRESPDATE | getStartHours() | Begin");
...
function getEndHours(workDate, calName, shift, org) {
	...
	//myLogger.debug(">>>>>  EX_SRRESPDATE | getEndHours() | Begin");
...
function calcRem(tdiff) {
	//myLogger.debug(">>>>>  EX_SRRESPDATE | calcRem() | Begin");
	secondinmillis = 1000;
...
function ISODateString(d) {
	//myLogger.debug(">>>>>  EX_SRRESPDATE | ISODateString() | Begin");
	function pad(n) {
		return n < 10 ? '0' + n : n
	}
