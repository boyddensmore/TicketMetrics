...
function calcBusTime(startDate, endDate) {
	...
	if (slaRecordSet.isEmpty() || slaRecordSet.count() < 1) {
		...
		// myLogger.debug(">>>>>  EX_SRSTATUS | calcBusTime() | slaRecordMbo.getMboValue(SLANUM): " + slaRecordMbo.getMboValue("SLANUM"));
		// myLogger.debug(">>>>>  EX_SRSTATUS | calcBusTime() | slaRecordMbo.getMboValue(CALCORGID): " + slaRecordMbo.getMboValue("CALCORGID"));
		// myLogger.debug(">>>>>  EX_SRSTATUS | calcBusTime() | slaRecordMbo.getMboValue(CALCCALENDAR): " + slaRecordMbo.getMboValue("CALCCALENDAR"));
		// myLogger.debug(">>>>>  EX_SRSTATUS | calcBusTime() | slaRecordMbo.getMboValue(CALCSHIFT): " + slaRecordMbo.getMboValue("CALCSHIFT"));
...
function getHours(startDt, endDt, calName, shift, org) {
	...
	//myLogger.debug(">>>>>  EX_SRSTATUS | getHours() | startDt: " + startDt);
	//myLogger.debug(">>>>>  EX_SRSTATUS | getHours() | endDt: " + endDt);
	//myLogger.debug(">>>>>  EX_SRSTATUS | getHours() | calName: " + calName);
	//myLogger.debug(">>>>>  EX_SRSTATUS | getHours() | shift: " + shift);
	//myLogger.debug(">>>>>  EX_SRSTATUS | getHours() | org: " + org);
